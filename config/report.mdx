...
This is why we're always looking for new resources to help us keep up, and that's where the State of CSS survey comes in. This year again, we aggregated a ton of data from developers across the world that really highlights the changes that the CSS world is going through right now. While we encourage you to check out the [full results](/), we also wanted to compile this quick overview of a few major trends.

But first, a little more information about who took the survey.
...
<Block id="features_overview" size="l" title="New CSS Features">
    ...
    <Overlay id={['prefers_reduced_motion', 'prefers_color_scheme']}>
        And so do new media queries like `prefers-reduced-motion` (**15%** usage) and
        `prefers-color-scheme` (**18%** usage).
    </Overlay>
</Block>
...
<Block
    ...
    title="Do you agree that CSS is a programming language?"
    size="s"
/>
